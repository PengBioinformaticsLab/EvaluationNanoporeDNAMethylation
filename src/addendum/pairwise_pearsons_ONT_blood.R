...
# File Loading
file_paths <- list.files(path = "../nanopore_epic/data", pattern = "*\\.rds$", full.names = TRUE, recursive = TRUE)
desired_order <- c("4059246541",  
                   "4059249500", 
                   "4059246577", 
                   "4059249475") 

# Extract sample IDs from file paths
sample_ids <- stringr::str_extract(basename(file_paths), "\\d+")

# Reorder
file_paths <- file_paths[match(desired_order, sample_ids)]

# Read all files into a list and name them automatically (e.g., Sample_1, Sample_2)
sample_names <- paste("Sample", c("A", "B", "C", "D"))
...
samples <- map(samples, ~ mutate(.x, beta = round((NMod + NOtherMod) / score, 4)))
...
write_csv(nano_tab, file.path(out_dir, "nanopore_blood_paired_pearsons.csv"))
